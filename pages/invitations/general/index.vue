          ...
          <div class="add">
            ...
            <form id="invitations" class="invitations" v-if="invit">
                ...
                <div class="body">
                  <div class="row">
                    <div class="col-12">
                      <div class="alert alert-danger" role="alert" v-if="invaild != false">
                        {{invaild }}
                      </div>
                    </div>
                    <div class="col-12 title">
                   ...
                   <div class="col-md-12 mt-3">
                      <div class="row">
                        ...
                        <div class="col-md-6">
                          <label for="is_confirmed" class="mb-1">{{ $t("Order_Status") }}</label>
                          <div>
                            <select class="is_confirmed" name="is_confirmed" v-model="formData.is_confirmed">
                            <option value="0">{{ $t("under_consideration") }}</option>
                            <option value="1">{{ $t("confirmed") }}</option>
                            <option value="null">{{ $t("Apologies") }}</option>
                          </select>
                          </div>
                        </div>
                      </div>
        ...
        <div class="col-12 control">
          <div class="row">
            ...
            <div class="col-lg-3 col-md-6 mt-2">
              ...
              <div class="col-lg-3 col-md-6 mt-2">
              <label for="filter_Whatsapp">{{ $t("phone_Number") }}</label>
              <div class="mt-2"><input type="text" v-model="filter_Whatsapp" name="filter_Whatsapp" class="form-control"></div>
            </div>
            <div class="col-lg-3 col-md-6 mt-2">
              <label for="filter_Class">{{ $t("Class") }}</label>
              <div class="mt-2">
                <select name="filter_Class" id="" v-model="filter_Class">
                    <option :value="``" >{{ $t("All") }}</option>
                    <option v-for="item in categoriesInvit" :key="item.id" :value="item.name" >{{item.name }}</option>
                </select>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 mt-2">
              <label for="filter_Status">{{ $t("Order_Status") }}</label>
              <div class="mt-2">
                <select name="filter_Status" id="" v-model="filter_Status">
                    <option :value="``" >{{ $t("All") }}</option>
                    <option :value="`0`" >{{ $t("under_consideration") }}</option>
                    <option :value="`1`" >{{ $t("confirmed") }}</option>
                    <option :value="`null`" >{{ $t("Apologies") }}</option>
            ...
            <div class="col-lg-3 col-md-6 mt-2">
              <label for="filter_internal">{{ $t("internal") }}/{{ $t("external") }}</label>
              <div class="mt-2">
                <select name="filter_internal" id="" v-model="filter_internal">
                    <option :value="``" >{{ $t("All") }}</option>
                    <option :value="`1`" >{{ $t("internal") }}</option>
                    <option :value="`0`" >{{ $t("external") }}</option>
                </select>
                </div>
            </div>
            <!-- <div class="col-12">
        ...
        <div class="col-12 list">
           <table>
              ...
              <tr v-for="(item,index) in filterByName" :key="item.id">
                ...
                <td>{{item.email}}</td>
                <td>
                    <span class="p-1" style="background: #66c;color:#fff" v-if="item.is_confirmed == 0">{{ $t("under_consideration") }}</span>
                    <span class="p-1" style="background: #38b96e;color:#fff" v-if="item.is_confirmed == 1">{{ $t("confirmed") }}</span>
                    <span class="p-1" style="background: #f44336;color:#fff" v-if="item.is_confirmed == null">{{ $t("Apologies") }}</span>
                </td>
                <td>
                  <span v-if="item.is_internal == 1">{{ $t("internal") }}</span>
                  <span v-else>{{ $t("external") }}</span>
                </td>
                <td>
      ...
      if (this.filter_Status != '') {
        return this.invitations.filter((item) => {
          return this.filter_Status
                       .toLowerCase()
                       .split(" ")
                       .every((v) => item.is_confirmed == v);
                }) 
      }
      if (this.filter_internal != '') {
        return this.invitations.filter((item) => {
          return this.filter_internal
                       .toLowerCase()
                       .split(" ")
                       .every((v) => item.is_internal == v);
                }) 
      }
  ...
  return yyyy + '-' + (mmChars[1]?mm:"0"+mmChars[0]) + '-' + (ddChars[1]?dd:"0"+ddChars[0]);
   ...
   add(){
    ...
    showPut(item){
      ...
      self.formData.is_whatsapp = item.is_whatsapp;
      self.formData.is_attendant = item.is_attendant;
      self.formData.is_confirmed = item.is_confirmed;
      self.formData.lang = item.lang;
      self.formData.title.name = item.title.name;
    },
    edit(){
